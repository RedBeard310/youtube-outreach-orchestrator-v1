  ...
  try {
    ...
    if (bookDrained) {
      ...
      appendFileSync(OBSERVATIONS, JSON.stringify({
        ...
        detail: `The recovery lane's collect pool is down to ${depth.pool} leads while the lane walks ${perDay} lead-slots a day, so it re-walks its whole remaining book about ${laps}x daily. This — not website resolution — is what caps its output, and the two site-resolution alarms are suppressed this run because they would name a search-plan top-up as the remedy. Raising the Brave cap buys ${depth.pool} leads, not the needs_contact pool. The collect/verify gap that hid 2,780 leads behind this same alarm was closed on 2026-09-13, so a drained pool now means the lane has genuinely worked its book rather than that a selector is hiding it; ${depth.stranded} leads still hold a non-email contact point and cannot be collected, and that number climbing is the signal a gap has reopened. Check whether new needs_contact arrivals have stopped before spending anything.`,
      }) + '\n');
